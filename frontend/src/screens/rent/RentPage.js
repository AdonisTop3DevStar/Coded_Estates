import { Row, Col, Button, Image, Modal, Container, Nav, Tab, ButtonGroup, Form } from "react-bootstrap";
...
function RentPage() {
  ...
  const findProperties = (location, adult, dates, children, infants, pets, keyword) => {
    if (keyword == "" && location == '' && dates[0]?.toDate?.() === undefined && dates[1]?.toDate?.() === undefined && adult == 0 && children == 0 && infants == 0 && pets == undefined) {
      let tempData = [];
      ...
      if (keyword !== "") {
        result = result.filter(obj => (obj.address.state).toLowerCase().includes(keyword.toLowerCase()) || (obj.address.city).toLowerCase().includes(keyword.toLowerCase()) || (obj.address.streetAddress).toLowerCase().includes(keyword.toLowerCase()) || (obj.address.area).toLowerCase().includes(keyword.toLowerCase()));        
      }
      ...
      <div className="SearchBar border-bottom">
        <Container className="d-flex justify-content-center">
          <div className="my-2 p-2 border rounded-5 d-flex align-items-center">
            ...
            <div className="py-1 px-3 fw-bold border-end cursor-pointer" onClick={() => (setTimeShow(true))}>
            ...
            {adult == 0 && children == 0 && infants == 0 && pets == 0 ? ("Add Guests") : (
                <div className=" d-flex flex-column">
                  ...
                  <div className="location-search-b">
                    {(adult > 0 || children > 0 ) && (<span className="me-2">{adult + children + "-guests"}</span>)}
                    {infants > 0 && (<span className="me-2">{infants + "-infants"}</span>)}
